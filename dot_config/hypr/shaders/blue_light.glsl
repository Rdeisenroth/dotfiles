...
const float temperature=3600.;
const float temperatureStrength=1.;
...
vec3 colorTemperatureToRGB(const in float temperature){
    ...
    return mix(
        clamp(vec3(m[0]/(vec3(clamp(temperature,1000.,40000.))+m[1])+m[2]),vec3(0.),vec3(1.)),
        vec3(1.),
        smoothstep(1000.,0.,temperature)
    );
}
...
void main(){
    ...
    color*=mix(1.,
        dot(color,vec3(.2126,.7152,.0722))/max(dot(color,vec3(.2126,.7152,.0722)),1e-5),
